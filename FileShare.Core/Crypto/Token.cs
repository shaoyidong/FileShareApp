...
namespace FileShare.Core.Crypto
{
    ...
    public static class Token
    {
        ...
        public static string GenerateTokenNonce(SigningTokenKey key, byte[] salt)
        {
            ...
            var hashBase64 = Base64UrlUtil.Encode(digest);
            var saltBase64 = Base64UrlUtil.Encode(salt);
            var signatureBase64 = Base64UrlUtil.Encode(signature);
            ...
            Action<byte[]> verifySalt)
        {
            ...
            try
            {
                saltBytes = Base64UrlUtil.Decode(saltBase64);
            ...
            if (Base64UrlUtil.Encode(expectedDigest) != hashBase64)
            {
                return false;
            }
            ...
            try
            {
                signature = Base64UrlUtil.Decode(signatureBase64);
            }
            catch
            {
                return false;
